define([
  ...
  'document/annotation/image/selection/layers/linkedbox/linkedBoxLayer',
    ...
    var layers = {
      ...
      getAnnotationAt = function(e) {
        ...
        jQuery.each(layers, function(key, layer) {
          ...
          if (result) allAnnotations.push(result);
        });
        ...
        if (allAnnotations.length > 0) return allAnnotations[0];
      },
      /** @override **/
      ...
      setOverlayColor = function(color) {
        for (var key in layers) layers[key].setColor(color);
      },
      toggleOverlays = function() {
        for (var key in layers) layers[key].toggleVisibility();
      };
