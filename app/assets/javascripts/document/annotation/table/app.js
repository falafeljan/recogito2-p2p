...
require([
  ...
  var App = function(results) {
    var self = this,
      ...
      frozenColumns = [
        {
          ...
          field: '__annotation',
          width: 145,
          formatter: Highlighter.CellFormatter,
          resizable: false,
        },
      ...
      onScroll = function(e, args) {
        ...
        if (selection) {
          if (isSelectionVisible(grid.getSelectedRows())) setClipping(false);
          else setClipping(true);
    ...
    PlaceUtils.initGazetteers().done(function() {
      API.listAnnotationsInPart(Config.documentId, Config.partSequenceNo)
        ...
        .fail(self.onAnnotationsLoadError.bind(self))
        .then(loadIndicator.destroy);
    });
  };
  ...
  jQuery(document).ready(function() {
    var isHTTPS = location.protocol === 'https:',
      dataURL = jsRoutes.controllers.document.DocumentController.getRaw(
        Config.documentId,
